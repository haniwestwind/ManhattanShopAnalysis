...
def filter_and_save_manhattan_data():
    ...
    manhattan_mask, df_manhattan = filter_manhattan_businesses_with_shapefile(df_stores)

    non_manhattan_mask = ~manhattan_mask

    # Delete non-Manhattan businesses from the database
    non_manhattan_ids = df_stores[non_manhattan_mask]["_id"].tolist()
    if non_manhattan_ids:
        result = businesses_collection.delete_many({"_id": {"$in": non_manhattan_ids}})
        print(f"Deleted {result.deleted_count} non-Manhattan businesses from the database.")
    
    # Check if they are deleted
    store_data_again = list(businesses_collection.find())
    df_stores_after_delete = pd.DataFrame(store_data_again)
    ...
    visualize_manhattan_stores(df_stores_after_delete)
...
def visualize_manhattan_stores(df_manhattan):
    ...
    for _, store in df_manhattan.iterrows():
        ...
        folium.CircleMarker(
            ...
            radius=store["rating"] * 4,  
            color="red",
            fill=True,
            fill_color="lightcoral",
            fill_opacity=0.6,
            popup=f"<b>{store['name']}</b><br>Rating: {store['rating']}<br>Reviews: {store['review_count']}",
        ).add_to(nyc_map)
    ...
    map_filename = os.path.join(save_dir, "check_delete_stores_map.html")
