...
The dataset for this project was primarily sourced from Yelp's business database, which provided essential attributes such as business ratings, review counts, category labels, and location information (latitude, longitude, and zip code). However, to enhance the analysis, several external datasets were integrated to capture environmental and socioeconomic factors that may influence business success. These additional datasets included Subway Station Data (Obtained from the MTA dataset, which included station names, latitude and longitude coordinates, and subway lines), Public Parks Data (Provided by NYC Open Data, containing geospatial representations of park locations and their respective boroughs),  Police Precinct Data (Extracted from NYC law enforcement records, detailing the locations of police precincts and their jurisdictional boundaries), Rat Sighting Reports (Derived from NYC 311 complaint data, indicating locations with reported rodent activity), Public Restrooms Data (NYC government records detailing the locations of public restrooms, their accessibility status, and hours of operation), School Locations Data (NYC Department of Education data listing schools, their latitude and longitude, and their educational levels), Census Income Data (Sourced from NYC census data, containing average household income statistics per zip code), Complaints (Sourced from NYC 311 and assess neighborhood issues from misdemeanors to felonies). To efficiently manage and merge these diverse data sources, I used MongoDB as a central database to seamlessly extract features and retrieve different datasets.
